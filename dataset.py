...
def collate_fn_override(data):
    ...
    #     print("Data arr 0 shape = ", data_arr[0].shape)
    #     print("Data arr 1 shape =", data_arr[1].shape)
    #     print("Data arr 0 type = ", data_arr[0].dtype)
...
class AugmentDataset(torch.utils.data.Dataset):
    def __init__(self, args, fold, fold_file_name, augment=False,
    ...
    def getitem(self, index):  # Try to use this for debugging purpose
        ...
        if np.random.randint(low=0, high=2)==0 and (not self.validation) and (self.augment):
