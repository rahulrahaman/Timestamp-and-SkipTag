...
would be: 
python runcodes/run_em_single_frame.py --split 1 --select_f data/50salads_random19frame_selection.pkl 
...
def get_out_dir():
    ...
    args.output_dir = args.base_dir + f"results/em_algo/sf_{args.select_f.split('/')[-1].split('.')[0]}"
    if args.use_mse:
    ...
    posterior_path = os.path.join(args.output_dir, "posterior_weights")
...
def model_pipeline():
    global boundary_frames_dict, selected_frames_dict, mat_poisson, loaded_mean_var_actions
    global args
    args = get_out_dir()
    ...
    loaded_mean_var_actions = pickle.load(open(args.mean_var_f, "rb"))
    get_label_idcsv()
...
def prob_vals_per_segment(selected_frames, cur_vid_feat, labels, vidid, gt_labels):
    ...
    first_idx, last_idx = selected_frames[0], selected_frames[-1]
    first_label, last_label = labels[0], labels[-1]
    per_frame_weights[:first_idx, first_label] = 1.0
    per_frame_weights[last_idx:, last_label] = 1.0
    ...
    for i, cur_ele in enumerate(selected_frames[:-1]):
        ...
        indices = torch.arange(cur_ele, next_ele).long().to(cumsum_feat.device)
        ...
        minlen = torch.min(cur_boundary_len)
        maxlen = torch.min(cur_boundary_len) + 1
        len_prob = get_poisson_prob(minlen, maxlen, label_cur_ele)

        prob = torch.softmax((left_sum + right_sum + len_prob / args.prior_temp), dim=0)
        cumm_prob = torch.cumsum(prob, dim=0)
        per_frame_weights[cur_ele:next_ele, label_next_ele] = cumm_prob - prob
        per_frame_weights[cur_ele:next_ele, label_cur_ele] = 1. - per_frame_weights[cur_ele:next_ele, label_next_ele]

        cur_boundary = round(np.sum(np.arange(cur_ele, next_ele, 1) * prob.detach().cpu().numpy()).item())
        boundary_error += (boundary_frames_dict[vidid + '.txt'][num_boundary] - cur_boundary)**2
        num_boundary += 1
...
def calculate_element_probb(data_feat, data_count, video_ids, gt_labels):
    ...
    for iter_num in range(len(data_count)):
        ...
        with torch.no_grad():
            # Multi-processing
            collect_result(prob_vals_per_segment(selected_frames_indices, cur_vid_feat,
                                                 selected_frames_labels, cur_vidid, cur_gt_labels))
...
def confidence_loss(probs, vid_ids, vidlens):
    ...
    for proba, idx, vidlen in zip(probs, vid_ids, vidlens):
        ...
        loss += torch.sum(right_penalty)/torch.sum(mask_right[1:])
        left_penalty = torch.clamp(-transition * mask_left[1:], min=0)
        loss += torch.sum(left_penalty)/torch.sum(mask_left[1:])
    return loss/bsize
...
def make(model, device):
    ...
    print(args)
    traindataset = BreakfastWithWeights(args, fold='train', fold_file_name=args.train_split_file, sample_rate=args.sample_rate)
...
def main():
    ...
    if not args.notrain:
        wandb.init(project=f'EM single frame {args.dataset_name}-split{args.split}', config=args, entity='rahul-dipika')
    model_pipeline()
