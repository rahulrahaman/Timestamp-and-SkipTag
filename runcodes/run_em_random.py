...
my_parser.add_argument('--weak_labels_f', type=str, required=False)
...
def get_out_dir():
    ...
    args.output_dir = args.base_dir + "results/em_algo/gen_{}_init{}".format(
        args.select_f.split("/")[-1].split(".")[0], args.init_epoch)
...
def model_pipeline():
    ...
    if args.resume_model is not None:
        ...
        print(f"Loaded model with successfully from path {args.resume_model}")
    ...
    if args.resume_model is not None:
        ...
        val_acc, best_val_acc, final_scores = validate(args, model, test_loader, device)
    
    if not args.notrain:
        model = train(device, model, train_loader_list, criterion_list, optimizer, args,
                      test_loader, get_single_random, perform_expectation)
...
def prob_vals_per_segment(selected_frames, cur_vid_feat, labels, first_ele_flag, last_ele_flag, vidid, gt_labels):
    ...
    boundary_error = 0
    num_boundary = 0
    labels = [args.num_class-1] + labels if selected_frames[0] != 0 else labels
    labels = labels + [args.num_class-1] if selected_frames[-1] != num_frames-1 else labels
...
#             pdb.set_trace()
        ...
        if label_cur_ele != label_next_ele:
            left_sum = (cumsum_feat[b1, label_cur_ele] - strt_index)
            mat_b1_b2_c_prob[b1-cur_ele, b1-cur_ele, dummy_label] = left_sum / args.temp + b1_prior[b1-cur_ele]
        else:
            # returns prob that the left class length >= seg len
            b1_prior_ = get_poisson_logcdf(next_ele - prev_boundary, label_cur_ele) 
            mat_b1_b2_c_prob[b1-cur_ele, b1-cur_ele, dummy_label] = left_sum  / args.temp + b1_prior_
...
def main():
    ...
    if not args.notrain:
        wandb.init(project='EM Single-frame-supervision', config=args, entity='rahul-dipika')
